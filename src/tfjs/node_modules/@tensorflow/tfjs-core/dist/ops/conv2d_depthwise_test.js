...
test_util_1.describeWithFlags('depthwiseConv2D', test_util_1.ALL_ENVS, function () {
    it('input=1x3x3x1,f=2,s=1,d=1,p=valid,chMul=1', function () {
        ...
        var w = dl.tensor4d([0.303873, 0.229223, 0.144333, 0.803373], [fSize, fSize, inDepth, chMul]);
        var result = dl.depthwiseConv2d(x, w, stride, pad);
    ...
    it('input=1x3x3x1,f=2,s=1,d=2,p=valid,chMul=1', function () {
        ...
        var wDilated = dl.tensor4d([0.303873, 0, 0.229223, 0, 0, 0, 0.144333, 0, 0.803373], [fSizeDilated, fSizeDilated, inDepth, chMul]);
        var result = dl.depthwiseConv2d(x, w, stride, pad, 'NHWC', dilation);
        var expectedResult = dl.depthwiseConv2d(x, wDilated, stride, pad);
    ...
    it('input=1x3x3x2,f=2,s=1,d=2,p=same,chMul=1', function () {
        ...
        var w = dl.stack([
            dl.tensor2d([0.614293, 0.0648011, 0.101113, 0.452887], [fSize, fSize]),
            dl.tensor2d([0.0582746, 0.426481, 0.872743, 0.765767], [fSize, fSize])
        ], 2)
            .expandDims(3);
        var fSizeDilated = fSize + (fSize - 1) * (dilation - 1);
        var wDilated = dl.stack([
            dl.tensor2d([0.614293, 0, 0.0648011, 0, 0, 0, 0.101113, 0, 0.452887], [fSizeDilated, fSizeDilated]),
            dl.tensor2d([0.0582746, 0, 0.426481, 0, 0, 0, 0.872743, 0, 0.765767], [fSizeDilated, fSizeDilated])
        ], 2)
            .expandDims(3);
    ...
    it('input=2x3x3x2,f=2,s=1,d=2,p=same,chMul=2', function () {
        ...
        var w = dl.stack([
            dl.stack([
                dl.tensor2d([0.240347, 0.906352, 0.478657, 0.825918], [fSize, fSize]),
                dl.tensor2d([0.380769, 0.184705, 0.238241, 0.201907], [fSize, fSize])
            ], 2),
            dl.stack([
                dl.tensor2d([0.294087, 0.181165, 0.191303, 0.7225], [fSize, fSize]),
                dl.tensor2d([0.430064, 0.900622, 0.670338, 0.33478], [fSize, fSize])
            ], 2)
        ], 3);
        var fSizeDilated = fSize + (fSize - 1) * (dilation - 1);
        var wDilated = dl.stack([
            dl.stack([
                dl.tensor2d([0.240347, 0, 0.906352, 0, 0, 0, 0.478657, 0, 0.825918], [fSizeDilated, fSizeDilated]),
                dl.tensor2d([0.380769, 0, 0.184705, 0, 0, 0, 0.238241, 0, 0.201907], [fSizeDilated, fSizeDilated])
            ], 2),
            dl.stack([
                dl.tensor2d([0.294087, 0, 0.181165, 0, 0, 0, 0.191303, 0, 0.7225], [fSizeDilated, fSizeDilated]),
                dl.tensor2d([0.430064, 0, 0.900622, 0, 0, 0, 0.670338, 0, 0.33478], [fSizeDilated, fSizeDilated])
            ], 2)
        ], 3);
        var result = dl.depthwiseConv2d(x, w, stride, pad, 'NHWC', dilation);
        var expectedResult = dl.depthwiseConv2d(x, wDilated, stride, pad, 'NHWC', noDilation);
        expect(result.shape).toEqual(expectedResult.shape);
        test_util_1.expectArraysClose(result, expectedResult);
    });
    ...
    it('Pass null for dilations, which defaults to [1, 1]', function () {
        ...
        var x = dl.zeros([3, 3, inDepth]);
        var w = dl.zeros([fSize, fSize, inDepth, chMul]);
        var result = dl.depthwiseConv2d(x, w, stride, pad, 'NHWC', dilations);
        expect(result.shape).toEqual([3, 3, inDepth * chMul]);
    });
});
