...
test_util_1.describeWithFlags('separableConv2d', test_util_1.ALL_ENVS, function () {
    it('input=1x3x3x1,f=2,s=1,d=1,p=valid,chMul=1,outDepth=2', function () {
        ...
        var depthwiseFilter = dl.tensor4d([0.303873, 0.229223, 0.144333, 0.803373], [fSize, fSize, inDepth, chMul]);
        var pointwiseFilter = dl.tensor4d([0.1, -0.2], [1, 1, inDepth * chMul, outDepth]);
        var result = dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, stride, pad);
        test_util_1.expectArraysClose(result, dl.tensor4d([
    ...
    it('input=1x3x3x1,f=2,s=1,d=1,p=valid,chMul=2,outDepth=2', function () {
        ...
        var depthwiseFilter = dl.tensor4d([
            0.303873, 0.229223, 0.144333, 0.803373, -0.303873, -0.229223,
            -0.144333, -0.803373
        ], [fSize, fSize, inDepth, chMul]);
        var pointwiseFilter = dl.tensor4d([0.1, -0.2, -0.1, 0.2, 0.15, 0.15], [1, 1, inDepth * chMul, outDepth]);
        var result = dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, stride, pad);
        test_util_1.expectArraysClose(result, dl.tensor4d([
    ...
    it('input=1x3x3x1,f=2,s=1,d=1,p=valid,chMul=1,outDepth=2,3D input', function () {
        ...
        var pointwiseFilter = dl.tensor4d([0.1, -0.2], [1, 1, inDepth * chMul, outDepth]);
        var result = dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, stride, pad);
        test_util_1.expectArraysClose(result, dl.tensor3d([
            0.10702161, -0.21404321, 0.10316753, -0.20633507, 0.06704096,
            -0.13408193, 0.07788632, -0.15577264
        ], [2, 2, outDepth]));
    });
    ...
    it('input=1x4x4x2,f=2,s=2,d=1,p=valid,chMul=1,outDepth=2', function () {
        ...
        var pointwiseFilter = dl.tensor4d([0.1, -0.2, 0.05, -0.05], [1, 1, inDepth * chMul, outDepth]);
        var result = dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, stride, pad);
        test_util_1.expectArraysClose(result, dl.tensor4d([
    ...
    it('input=1x4x4x1,f=2,s=1,d=2,p=valid,chMul=1,outDepth=2', function () {
        ...
        var depthwiseFilter = dl.tensor4d([0.303873, 0.229223, 0.144333, 0.803373], [fSize, fSize, inDepth, chMul]);
        var pointwiseFilter = dl.tensor4d([0.1, -0.2], [1, 1, inDepth * chMul, outDepth]);
        var result = dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, stride, pad, dilationRate);
        test_util_1.expectArraysClose(result, dl.tensor4d([
    ...
    it('Incorrect input rank raises error', function () {
        var x = dl.zeros([4, 4]);
        var depthwiseFilter = dl.zeros([2, 2, 1, 3]);
        var pointwiseFilter = dl.zeros([1, 1, 2, 4]);
        expect(function () {
            return dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, 1, 'valid');
        })
            .toThrowError(/rank 4/);
    });
    it('Incorrect depthwise filter rank raises error', function () {
        var x = dl.zeros([1, 4, 4, 1]);
        var depthwiseFilter = dl.zeros([2, 2, 1]);
        var pointwiseFilter = dl.zeros([1, 1, 2, 4]);
        expect(function () {
            return dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, 1, 'valid');
        })
    ...
    it('Incorrect depthwise filter rank raises error', function () {
        var x = dl.zeros([1, 4, 4, 1]);
        var depthwiseFilter = dl.zeros([2, 2, 1, 3]);
        var pointwiseFilter = dl.zeros([1, 1, 2]);
        expect(function () {
            return dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, 1, 'valid');
        })
            .toThrowError(/rank 4/);
    ...
    it('Incorrect point filter 1st dimension raises error', function () {
        var x = dl.zeros([1, 4, 4, 1]);
        var depthwiseFilter = dl.zeros([2, 2, 1, 3]);
        var pointwiseFilter = dl.zeros([2, 1, 3, 6]);
        expect(function () {
            return dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, 1, 'valid');
        })
    ...
    it('Incorrect point filter 2nd dimension raises error', function () {
        var x = dl.zeros([1, 4, 4, 1]);
        var depthwiseFilter = dl.zeros([2, 2, 1, 3]);
        var pointwiseFilter = dl.zeros([1, 5, 3, 6]);
        expect(function () {
            return dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, 1, 'valid');
        })
    ...
    it('Incorrect pointwise filter 3rd dimension raises error', function () {
        var x = dl.zeros([1, 4, 4, 1]);
        var depthwiseFilter = dl.zeros([2, 2, 1, 3]);
        var pointwiseFilter = dl.zeros([1, 1, 4, 6]);
        expect(function () {
            return dl.separableConv2d(x, depthwiseFilter, pointwiseFilter, 1, 'valid');
        })
