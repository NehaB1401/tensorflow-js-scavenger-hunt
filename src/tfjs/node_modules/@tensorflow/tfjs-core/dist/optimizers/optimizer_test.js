...
test_util_1.describeWithFlags('optimizer', test_util_1.ALL_ENVS, function () {
    it('basic', function () {
        ...
        var bias = dl.scalar(1).variable();
        var strayVariable = dl.scalar(-1).variable();
    ...
    it('only bias trainable', function () {
        ...
        var optimizer = new sgd_optimizer_1.SGDOptimizer(learningRate);
        var trainable = false;
        var x = dl.scalar(4).variable(trainable);
        var bias = dl.scalar(1).variable();
        var strayVariable = dl.scalar(-1).variable();
        var f = function () { return x.square().addStrict(bias); };
        var cost = optimizer.minimize(f, true);
