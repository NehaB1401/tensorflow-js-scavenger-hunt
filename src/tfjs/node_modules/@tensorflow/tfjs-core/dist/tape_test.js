...
test_util_1.describeWithFlags('getFilteredNodesXToY', test_util_1.CPU_ENVS, function () {
    it('getFilteredNodesXToY no paths from x to y', function () {
        var x = dl.scalar(1);
        var intermediate1 = dl.scalar(0);
        var intermediate2 = dl.scalar(0);
        var y = dl.scalar(2);
        var tape = [
            {
                id: 0,
...
test_util_1.describeWithFlags('backpropagateGradients', test_util_1.CPU_ENVS, function () {
    ...
    it('basic backprop with 1 node', function () {
        ...
        var tape = [{
                ...
                gradient: function (dy) {
                    return { x: function () { return dy.add(dl.scalar(1)); } };
                }
            }];
    ...
    it('basic backprop with 2 nodes', function () {
        ...
        var tape = [
            ...
            },
            {
                ...
                output: y,
                gradient: function (dy) {
                    return { intermediate: function () { return dy.add(dl.scalar(1)); } };
                }
            }
        ];
        tape_util.backpropagateGradients(accumulatedGradientsMap, tape);
    ...
    it('basic backprop with a split node accumulates gradients', function () {
        var x = dl.scalar(0);
        var intermediate1 = dl.scalar(1);
        var intermediate2 = dl.scalar(2);
        var y = dl.scalar(3);
        var dy = dl.scalar(1);
        var accumulatedGradientsMap = {};
        accumulatedGradientsMap[y.id] = dy;
        var tape = [
            ...
            },
            {
                ...
                gradient: function (dy) {
                    return {
                        intermediate1: function () { return dy.add(dl.scalar(1)); },
                        intermediate2: function () { return dy.add(dl.scalar(1)); }
                    };
                }
            }
        ];
